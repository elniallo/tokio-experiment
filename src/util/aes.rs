...
pub fn encrypt_aes(data: &[u8], key: &[u8], iv: &[u8], extra: bool) -> Result<Vec<u8>, SymmetricCipherError> {
...
pub fn decrypt_aes(data: &[u8], key: &[u8], iv: &[u8], size: usize, extra: bool) -> Result<Vec<u8>, SymmetricCipherError> {
...
mod tests {
    ...
    fn it_encrypts_data_less_than_32_bytes() {
        ...
        let encryption = encrypt_aes(data, &key[..], &iv, true).unwrap();
    ...
    fn it_decrypts_encrypted_data() {
        ...
        let encryption = encrypt_aes(&data, &key[..], &iv, true).unwrap();
        let decryption = decrypt_aes(&encryption, &key[..], &iv, data.len(), true).unwrap();
