...
pub fn encrypt_aes(data: &[u8], key: &[u8], iv: &[u8], cipher: String) -> Result<Vec<u8>, Box<Error>> {
...
pub fn decrypt_aes(data: &[u8], key: &[u8], iv: &[u8], cipher: String) -> Result<Vec<u8>, Box<Error>> {
    let nid = get_nid(cipher)?;
    let algo = Cipher::from_nid(nid);
