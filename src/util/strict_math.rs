...
impl Add<StrictU64> for StrictU64 {
    ...
    fn add(self, rhs: StrictU64) -> Self::Output {
        if self.0 > (MAX - rhs.0) || rhs.0 > (MAX - self.0) {
            return Err(Box::new(Exception::new(&format!("Overflow: {} + {} will exceed maximum u64 size", self.0, rhs.0))));
        }
...
impl Sub<StrictU64> for StrictU64 {
    ...
    fn sub(self, rhs: StrictU64) -> Self::Output {
        if self.0 < rhs.0 {
            return Err(Box::new(Exception::new(&format!("Underflow: {} - {} will assign a negative value to an unsigned integer", self.0, rhs.0))));
        }
        Ok(StrictU64(self.0 - rhs.0))
    }
}
