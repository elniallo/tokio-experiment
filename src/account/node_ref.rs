...
impl NodeRef {
    pub fn new(address: &Vec<u8>, child: &Vec<u8>) -> NodeRef {
        NodeRef {
...
impl Proto for NodeRef {
    ...
    fn to_proto(&self) -> Result<Self::ProtoType, Box<Error>> {
        let mut proto_node_ref = Self::ProtoType::new();
        proto_node_ref.set_address(self.address.clone());
...
mod tests {
    ...
    fn it_makes_a_node_ref_from_inode_ref() {
        ...
        assert_eq!(node_ref.address, vec![109]);
        assert_eq!(
            node_ref.child,
    ...
    fn it_encodes_like_javascript_for_zero() {
        ...
        let encoding = node_ref.encode().unwrap();
        let javascript_encoding = vec![
            10, 1, 0, 18, 32, 0, 28, 0, 193, 0, 226, 0, 56, 0, 123, 0, 237, 0, 5, 0, 194, 0, 184,
            0, 131, 0, 152, 0, 186, 0, 70, 0, 162, 0, 115, 0, 42,
        ];
