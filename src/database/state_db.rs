...
impl<DatabaseType, EntryType, OptionType> StateDB<DatabaseType, EntryType>
    where DatabaseType: IDB<OptionType = OptionType>{
    pub fn new(path: PathBuf, options: Option<OptionType>) -> Result<StateDB<DatabaseType, EntryType>, Box<Error>> {
...
mod tests {
    ...
    use serialization::state::Data as ProtoData;
